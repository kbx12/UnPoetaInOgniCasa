...
- [ ] implementare la funzione `int CalcolaCompatibilita(char* verso1, char* verso2)` che dati due versi ritorna `1` se essi sono compatibili (vedi problema) o `0` se non lo sono.
...
- [ ]  implementare la funzione `char* CercaRima(char* verso, char** database)` che dato un verso e un array di stringhe (il database (vedi problema)) ritorna la prima stringa che è in rima con il verso oppure un puntatore nullo se non esiste tale stringa.
...
- [ ] implementare la funzione `char* CercaCesura(char** database)` che dato un array di stringhe (il database (vedi problema)) ritorna la prima stringa che è cesurabile(?) oppure un puntatore nullo.
...
- [ ] implementare la funzione `char* CercaAssonanza(char* verso, char** database)` che dato un verso ed un array di stringhe (il database (vedi problema)) ritorna la prima stringa assonante con il verso dato oppure un puntatore nullo se tale stringa non esiste.
- [ ] implementare la funzione `int CheckAlliterazione(char* verso)` che dato un verso ritorna `1` se il verso contiene una alliterazione (vedi problema) o `0` se non la contiene.
- [ ] implementare la funzione `char* CercaAlliterazione(char lettera, char** database)` che dato un array di stringhe (il database (vedi problema)) ed una lettera ritorna la prima stringa nel database che ha una alliterazione con quella lettera oppure un puntatore nullo. 
- [ ] implementare la funzione di supporto `float CalcolaPercentuale(int n1, int n2)` che dati due numeri calcola la percentuale di uno rispetto all'altro e la ritorna. (`5` è il `50%` di `10`).
- [ ] implementare la funzione di supporto `char** CaricaDatabase(const char* path_to_database)` che dato un percorso ad un file ritorna un array di stringhe dove ogni entrata corrisponde ad una riga del file.

### Come compilare

Per compilare il progetto da Windows bisogna entrare con il terminale nella cartella del progetto e poi scrivere il comando:
