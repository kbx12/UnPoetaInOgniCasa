...
- [ ]  implementare la funzione `char* CercaRima(char* verso, const char* path_to_database)` che dato un verso e il percorso relativo al database (vedi problema) ritorna la prima stringa che è in rima con il verso oppure un puntatore nullo se non esiste tale stringa.
...
- [ ] implementare la funzione `char* CercaCesura(const char* path_to_database)` che dato il percorso relativo al database (vedi problema) ritorna la prima stringa nel database che è cesurabile(?) oppure un puntatore nullo.
...
- [ ] implementare la funzione `char* CercaAssonanza(char* verso, const char* path_to_database)` che dato il percorso relativo al database (vedi problema) ritorna la prima stringa assonante nel database con il verso dato oppure un puntatore nullo se tale stringa non esiste.
...
- [ ] implementare la funzione `char* CercaAlliterazione(char lettera, const char* path_to_database)` che dato il percorso relativo al database (vedi problema) ed una lettera ritorna la prima stringa nel database che ha una allitterazione con quella lettera oppure un puntatore nullo. 
